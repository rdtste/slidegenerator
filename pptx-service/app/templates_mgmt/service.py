...
Consolidated from app.services.template_service into the templates_mgmt module.
Core operations: path resolution, PPTX loading, listing, inspection.
"""
...
def load_potx_as_presentation(path: Path) -> Presentation:
...
def list_templates() -> list[TemplateInfo]:
    ...
    if not templates:
        templates.append(get_default_template_info())
...
def load_presentation(template_id: str) -> Presentation:
    ...
    if path:
        ...
        if path.suffix.lower() == ".potx":
            return load_potx_as_presentation(path)
...
def inspect_template(path: Path) -> TemplateInfo | None:
    """Inspect a .pptx/.potx template and extract metadata."""
    try:
        if path.suffix.lower() == ".potx":
            prs = load_potx_as_presentation(path)
        else:
...
def get_default_template_info() -> TemplateInfo:
