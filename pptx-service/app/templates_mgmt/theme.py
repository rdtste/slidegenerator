"""Theme extraction — extracts visual properties from PowerPoint templates.

Consolidated from app.services.theme_service into the templates_mgmt module.
Provides lighter-weight theme extraction (vs. full profiling) and CSS generation.
"""
...
from app.templates_mgmt.profiler import parse_color_scheme
...
def classify_layout(layout_name: str) -> str | None:
...
def _estimate_capacity(width_cm: float, height_cm: float, font_pt: float) -> tuple[int, int]:
    ...
    line_height_cm = font_pt * 0.0353 * 1.5  # pt->cm x 1.5 spacing
