"""Template validator — pre-flight checks before generation (P1).

PPTX Skill requirement (Template Validation):
- Verify template exists and is valid
- Check template-to-design-system compatibility
- Validate template layout definitions
- Ensure required slide layouts are available
- Pre-check color viability in template
"""
...
class TemplateLayout(BaseModel):
    ...
    name: str  # "title", "content", "two-column", "image_text"
...
class TemplateValidator:
    ...
    P1 Enhancement: Ensures template is compatible with design system
    ...
    def validate_template(self, template_id: str) -> TemplateValidationResult:
        ...
        Returns:
            TemplateValidationResult with any issues found
        """
        issues = []
        metadata = None
    ...
    def _validate_file_integrity(self, template_path: Path) -> list[TemplateValidationIssue]:
        ...
        try:
            # PPTX is a ZIP file, attempt to validate
            import zipfile
